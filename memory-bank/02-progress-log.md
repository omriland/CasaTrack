...
- **Budget desktop grid:** [`VendorBudgetView`](src/components/renovation/views/VendorBudgetView.tsx) desktop uses **[AG Grid Community](https://www.ag-grid.com/react-data-grid/)** (`ag-grid-react` + `ag-grid-community` v35, Quartz theme): inline cell edit, full-width **Insert row** strips, pinned totals row, RTL, context menu; [`VendorBudgetDesktopGrid.tsx`](src/components/renovation/views/VendorBudgetDesktopGrid.tsx) + [`vendor-budget-grid-model.ts`](src/components/renovation/views/vendor-budget-grid-model.ts). Loaded with `next/dynamic` `ssr: false`. Mobile card layout unchanged.
- **Budget table UX (follow-up):** Full-page **`dir="rtl"`** + Hebrew copy; amount cells use **`dir="ltr"`** / `LtrAmount` so shekel amounts stay readable. **Sheets-style row insert:** hover the gap between rows (desktop `tr` + `colSpan` handle; mobile strip between cards) reveals indigo line + **+**; inserts draft **after** that row (or before all at top). Drafts carry **`insertAfter`**; `mergeTableRows` builds display order.
