    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 animate-pulse">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="h-24 bg-white rounded-xl border border-slate-200/60" />
          ))}
        </div>
      ) : rooms.length === 0 ? (
            ...
            {rooms.map((room) => {
              ...
              return (
                <button
                  ...
                  className={`text-left rounded-xl border p-4 transition-all shadow-sm active:scale-[0.98] ${
                    active
              ...
              <div className="p-5 space-y-4">
                <div>
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-3 py-2.5 rounded-lg border border-slate-200 bg-slate-50/50 text-[15px] text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 resize-none"
                  />
                </div>
                      ...
                      {roomTasks.map((t) => (
                        <li key={t.id}>
                          <Link
                            ...
                            className="block px-3 py-2 rounded-lg bg-slate-50 border border-slate-100 hover:bg-indigo-50/50 hover:border-indigo-100 text-[15px] font-medium text-slate-800 capitalize"
                            dir="auto"
                          >
                            {t.title}
                            {t.status !== 'done' && (
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
                      {roomPhotos.map((item) => (
                        <button
                          ...
                          className="aspect-square rounded-lg overflow-hidden bg-slate-100 border border-slate-200/60 active:scale-95 transition-all"
                        >
