  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end md:items-center justify-center p-0 md:p-4 bg-slate-900/40 backdrop-blur-sm transition-opacity" onClick={onClose}>
      <div
        ...
        className="w-full md:max-w-[480px] bg-white rounded-t-3xl md:rounded-2xl shadow-2xl overflow-hidden flex flex-col pt-2 md:pt-0 animate-fade-in-up md:animate-zoom-in"
      >
        {/* Header */}
        ...
        <form onSubmit={save} className="p-6 overflow-y-auto max-h-[85vh] space-y-6 pb-[max(1.5rem,env(safe-area-inset-bottom))]">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input dir="auto" value={category} onChange={e => setCategory(e.target.value)} placeholder="Materials..." className="w-full h-11 pl-10 pr-3 rounded-md border border-slate-200 bg-slate-50 text-[14px] font-semibold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm focus:bg-white" />
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <input dir="auto" value={payment} onChange={e => setPayment(e.target.value)} placeholder="Credit, Cash..." className="w-full h-11 pl-10 pr-3 rounded-md border border-slate-200 bg-slate-50 text-[14px] font-semibold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm focus:bg-white" />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
               ...
               <textarea dir="auto" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Add any extra details..." rows={2} className="w-full pl-10 pr-3 py-3 rounded-md border border-slate-200 bg-slate-50 text-[14px] font-semibold text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all shadow-sm focus:bg-white resize-none" />
            </div>
          </div>

          {/* Actions */}
          <div className="pt-4 flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 h-12 rounded-md bg-white border border-slate-200 text-slate-600 font-bold hover:bg-slate-50 active:scale-[0.98] transition-all shadow-sm">
              Cancel
            </button>
            <button type="submit" disabled={saving} className="flex-1 h-12 rounded-md bg-gradient-to-r from-indigo-600 to-indigo-500 text-white font-bold shadow-md hover:shadow-lg hover:from-indigo-500 hover:to-indigo-400 active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {saving ? (
