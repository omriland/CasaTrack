...
const primaryNav = [
  { href: '/renovation', label: 'Home', icon: HomeIcon, match: (p: string) => p === '/renovation' },
  {
    ...
    label: 'Tasks',
    icon: CheckIcon,
    match: (p: string) => p.startsWith('/renovation/tasks'),
  },
  {
    href: '/renovation/budget',
    label: 'Budget',
    icon: WalletIcon,
    match: (p: string) => p.startsWith('/renovation/budget'),
  },
  {
  ...
  {
    href: '/renovation/more',
    label: 'More',
    icon: MoreIcon,
    match: (p: string) => p === '/renovation/more' || moreMatches(p),
  },
] as const
            ...
            .map(item => {
              ...
              return (
                <Link
                  ...
                  className={`relative flex min-h-[60px] flex-1 flex-col items-center justify-center gap-0.5 transition-colors touch-manipulation select-none [-webkit-tap-highlight-color:transparent] ${
                    active
                      ? 'text-indigo-600'
  ...
  return (
    ...
    >
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M3 3h2l.4 2M7 13h10l3-8H6.4M7 13L5.4 5M7 13l-2 7h14M9 20a1 1 0 100 2 1 1 0 000-2zm8 0a1 1 0 100 2 1 1 0 000-2z"
      />
    </svg>
  )
}
